...
class RunLogger:
    ...
    def log_round_metrics(
        ...
        row = {
            ...
            "val_loss": "" if val_loss is None else f"{float(val_loss):.6f}",
            "val_acc": "" if val_acc is None else f"{float(val_acc):.6f}",
        }
...
#
def build_initial_params() -> List[np.ndarray]:
    import torch
    import torch.nn as nn
    import torch.nn.functional as F
    ...
    dataset = str(os.getenv("DATASET", "cifar10")).lower()
    if dataset in ("fashionmnist", "fashion-mnist"):
        m = SimpleFashionMNISTCNN()
    else:
        m = SimpleCifarCNN()
    return [v.detach().cpu().numpy() for _, v in m.state_dict().items()]
...
class ProxyOrchestratedFedAvg(fl.server.strategy.Strategy):
    def __init__(
        ...
        self.init_params_nd = build_initial_params()
    ...
    def _log_round_metrics(
        ...
        try:
            self.logger.log_round_metrics(
                r, selected, received, kept, dropped, agg_scalar,
    ...
    def aggregate_fit(
        ...
        for cp, fitres in results:
            ...
            nt = int(m.get("num_train", 0))
            nv = int(m.get("num_val", 0))

            tl = m.get("train_loss", None)
            ta = m.get("train_acc", None)
            vl = m.get("val_loss", None)
            va = m.get("val_acc", None)

            # 关键修改：增加对 acc 字段的严格非空检查
            if tl is not None and ta is not None and nt > 0:
                train_loss_sum += float(tl) * nt
                train_acc_sum += float(ta) * nt
                train_w_sum += nt
                
            if vl is not None and va is not None and nv > 0:
                val_loss_sum += float(vl) * nv
                val_acc_sum += float(va) * nv
                val_w_sum += nv

        train_loss = (train_loss_sum / train_w_sum) if train_w_sum > 0 else None
        train_acc = (train_acc_sum / train_w_sum) if train_w_sum > 0 else None
        val_loss = (val_loss_sum / val_w_sum) if val_w_sum > 0 else None
        val_acc = (val_acc_sum / val_w_sum) if val_w_sum > 0 else None
        ...
        self._log_event("AGG", server_round, {
        ...
        self._board_call("on_round_end")
        self._log_event("ROUND_END", server_round, {
            "kept": len(keep), "dropped": len(drop), "agg_scalar": agg_scalar, 
            "train_loss": train_loss, "train_acc": train_acc, 
            "val_loss": val_loss, "val_acc": val_acc
        })
